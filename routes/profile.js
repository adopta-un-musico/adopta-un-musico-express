...
router.post("/:currentUser", async (req, res, next) => {
  ...
  const { email, nickname, musicalGenres, instruments, loc } = req.body;
  try {
    ...
    // });
    const userProfile = await User.find({ nickname: currentUser });
    const user = await User.findByIdAndUpdate(userProfile[0]._id, {
      email,
      nickname,
      musicalGenres,
      instruments,
    ...
    res.redirect(`/profile/${userProfile[0].nickname}`);
  } catch (error) {
    next(error);
  }
});
